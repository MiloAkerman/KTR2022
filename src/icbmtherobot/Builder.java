...
public class Builder extends Unit {
        ...
        pathCallback = reason -> {
            if(reason == PATH_END_REASON.SUCCESSFUL) {
                if(mutating || rc.readSharedArray(Flag.BUILDER_POSTING.getValue()) != 0) {
                    ...
                    MapLocation nextBestLoc = getBestBuildLoc();
    ...
    public static MapLocation getBestBuildLoc() throws GameActionException {
        Pair<MapLocation, Integer> best = null;
        for(MapLocation loc : rc.getAllLocationsWithinRadiusSquared(rc.getLocation(), 20)) {
            if(best == null || rc.senseRubble(loc) < best.getValue()) {
                best = new Pair<>(loc, rc.senseRubble(loc));
            }
        }

        if(best != null) {
            System.out.println("Loc is " + best.getKey());
            return best.getKey();
        } else {
